...
class LaneDetector(object):
    def __init__(self, color, camera_type):
        # 车道线颜色
    ...
    def add_vertical_line_near(self, image):
        ...
        flip_binary = cv2.flip(roi, 1)  # 图像左右翻转
        # cv2.imshow('1', flip_binary)

        (x_1, y_1) = cv2.minMaxLoc(flip_binary)[-1]
        roi = flip_binary[y_1+ 45:y_1 + 55, :] 
        (x, y) = cv2.minMaxLoc(roi)[-1]
        up_p = (roi_w_max - (x_1 + x), y_1 + 45 + y)

        up_point = (0, 0)
        down_point = (0, 0)
        if up_p[1] - down_p[1] != 0 and up_p[0] - down_p[0] != 0:
            up_point = up_p#(int(-down_p[1]/((up_p[1] - down_p[1])/(up_p[0] - down_p[0])) + down_p[0]), 0)
            down_point = down_p
    ...
    def get_binary(self, image):
        ...
        mask = cv2.inRange(img_blur, tuple(lab_data['lab'][self.camera_type][self.target_color]['min']), tuple(lab_data['lab'][self.camera_type][self.target_color]['max']))  # 二值化
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if image is not None:
            ...
            up, down = lane_detect.add_vertical_line_near(binary_image)
            cv2.circle(img, up, 20, (255, 255, 255), -1)
            cv2.circle(img, down, 20, (255, 255, 255), -1)
